...
extern MPI_Comm comunicadorCarga; // Para la distribucion de la carga
extern MPI_Comm comunicadorCota;  // Para la difusion de una nueva cota superior detectada
...
extern int size;            // Numero de procesos que estan resolviendo el problema
...
void EquilibrioCarga(tPila *pila, bool *fin, tNodo *solucion)
{
  int solicitante;
  int n_nodos_recibidos;
  int flag;
  MPI_Status status;
  tPila pila_enviar;
  tNodo temp;
  ...
  if (pila->vacia())  // El proceso no tiene trabajo: pide a otros procesos
  {
    ...
    while (pila->vacia() && !(*fin))
    {
      ...
      case PETICION:  // Peticion de trabajo: recibimos peticion y la pasamos al siguiente proceso
        MPI_Recv(&solicitante, 1, MPI_INT, anterior, PETICION, comunicadorCarga, MPI_STATUS_IGNORE);
        MPI_Send(&solicitante, 1, MPI_INT, siguiente, PETICION, comunicadorCarga);

        if (solicitante == rank)  // Si recibimos el mismo mensaje que enviamos pasar token si lo tenemos
        {
          estado = PASIVO;

          if (token_presente)
          {
            if (rank == 0)  // Poner token al color del proceso
              color_token = BLANCO;
            else
              color_token = color;
      ...
      case NODOS: // Recibimos trabajo: guaramos nodos en pila y pasamos a activo
        MPI_Get_count(&status, MPI_INT, &n_nodos_recibidos);
        MPI_Recv(pila->nodos, n_nodos_recibidos, MPI_INT, status.MPI_SOURCE, NODOS, comunicadorCarga, MPI_STATUS_IGNORE);
        pila->tope = n_nodos_recibidos;
        estado = ACTIVO;
        break;

      case TOKEN: // Recibimos el token
        MPI_Recv(&color_token, 1, MPI_INT, siguiente, TOKEN, comunicadorCarga, MPI_STATUS_IGNORE);
        token_presente = true;

        if (estado == PASIVO)
        {
          if (rank == 0 && color == BLANCO && color_token == BLANCO)  // Fin detectado
          {
            *fin = true;

            // Mandar mensaje de fin junto a la solucion obtenida
            MPI_Send(solucion->datos, 2 * NCIUDADES, MPI_INT, siguiente, FIN, comunicadorCarga);

            // Recibimos confirmacion de que todos los procesos han acabado junto a la mejor solcion
            MPI_Recv(temp.datos, 2 * NCIUDADES, MPI_INT, anterior, FIN, comunicadorCarga, MPI_STATUS_IGNORE);

            if (temp.esMejorSolucion(*solucion))
              CopiaNodo(&temp, solucion);
          }
          else  // Seguimos pasando el token
          {
            if (rank == 0)
              color_token = BLANCO;
            else
      ...
      case FIN: // Fin del programa: recibimos solcuon del proceso anterior y actualizamos si es mejor
  ...
  if (!(*fin))  // Todavia tenemos nodos con los que trabajar
  {
    MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, comunicadorCarga, &flag, &status);

    while (flag > 0)  // Mientras haya mensajes pendientes
    {
      switch (status.MPI_TAG)
      {
      case PETICION:  // Peticion de otro proceso: dividimos pila y se la mandamos
        MPI_Recv(&solicitante, 1, MPI_INT, anterior, PETICION, comunicadorCarga, MPI_STATUS_IGNORE);

        if (pila->tamanio() > 1)  // Si hay suficientes nodos
        {
          pila->divide(pila_enviar);

          MPI_Send(pila_enviar.nodos, pila_enviar.tope, MPI_INT, solicitante, NODOS, comunicadorCarga);

          if (rank < solicitante) // Evitar situacion erronea de fin
            color = NEGRO;
        }
        else  // Si no hay suficientes nodos pasamos el mensaje
          MPI_Send(&solicitante, 1, MPI_INT, siguiente, PETICION, comunicadorCarga);

        break;

      case TOKEN: // Recibimos el token y nos lo guardamos
        MPI_Recv(&color_token, 1, MPI_INT, status.MPI_SOURCE, TOKEN, comunicadorCarga, MPI_STATUS_IGNORE);
        token_presente = true;
        break;
      }

      // Comprobar si aún hay mensajes pendientes
      MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, comunicadorCarga, &flag, &status);
    }
  }
}

void DifusionCotaSuperior(int &cota_superior, bool &hay_nueva_cota)
{
  int flag, nueva_cota;
  MPI_Status status;

  difundir_cs_local = hay_nueva_cota; // Indica si hay que difundir cota
  pendiente_retorno_cs = false; // Indica si tenemos que recibir la cota de otro proceso

  if (difundir_cs_local && !pendiente_retorno_cs) // Podemos difundir cota
  {
  ...
  while (flag > 0)
  {
    // Recibir cota de otro proceso
    MPI_Recv(&nueva_cota, 1, MPI_INT, anterior, status.MPI_TAG, comunicadorCota, MPI_STATUS_IGNORE);

    // Si es mejor actualizamos
    if (nueva_cota < cota_superior)
    {
      cota_superior = nueva_cota;
      hay_nueva_cota = true;
    }
    ...
    if (status.MPI_TAG == rank)
    {
      ...
      if (difundir_cs_local)  // Difundir la cota
      {
        MPI_Send(&cota_superior, 1, MPI_INT, siguiente, rank, comunicadorCota);
        pendiente_retorno_cs = true;
        difundir_cs_local = false;
      ...
      MPI_Send(&cota_superior, 1, MPI_INT, siguiente, status.MPI_TAG, comunicadorCota);
